...
namespace UnityEngine.U2D
{
    ...
    public class SpriteShapeObjectPlacement : MonoBehaviour
    {
        ...
        bool PlaceObjectOnHashChange()
        {
            ...
            {
                var count = 0;
                int spriteShapeHashCode = (int) 2166136261 ^ spriteShapeController.spline.GetHashCode();// spriteShapeController.splineHashCode; 
        ...
        {
            ShapeControlPoint[] shapePoints_t = new ShapeControlPoint[spriteShapeController.spline.GetPointCount()];
            for(int i=0; i<shapePoints_t.Length; i++)
            {
                var s = spriteShapeController.spline;
                ShapeControlPoint scp = new ShapeControlPoint();
                scp.position = s.GetPosition(i);
                scp.leftTangent = s.GetLeftTangent(i);
                scp.rightTangent = s.GetRightTangent(i);
                scp.mode = (int)s.GetTangentMode(i);

                shapePoints_t[i] = scp;

            }
            NativeArray<ShapeControlPoint> shapePoints = new NativeArray<ShapeControlPoint>(shapePoints_t, Allocator.Temp);

            //var shapePoints = spriteShapeController.GetShapeControlPoints(); // there's bug in here
        ...
        bool Place()
        {
            ...
            var closestPoint = Vector3.zero;
            {
                ...
                for (int i = 0; i < pointCount; ++i)
                {
                    ...
                    float t;
                    closestPoint = BezierUtility_2023.ClosestPointOnCurve(
                        position,
                        thisposition,
                        nextPosition,
                        rightTangent,
